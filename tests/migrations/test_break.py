...
async def notes(db):
    ...
    yield
    await OldNote.delete_all()
    await OldNote.get_async_pymongo_collection().drop()
    await OldNote.get_async_pymongo_collection().drop_indexes()
...
async def test_migration_break(settings, notes, db):
    ...
    notes = await OldNote.get_async_pymongo_collection().find().to_list(length=100)
